...
function connect(port,host,callback) {
	...
	self._debug(7,"Connecting to "+(host||"127.0.0.1")+":"+port);

	// Connect
	client = net.connect(port,host,function(){
		self._debug(7,"Connected to "+(host||"127.0.0.1")+":"+port);
		return self.addClient(client);
	})
	.on('error',function(err){
		self._debug(7,"Error connecting to "+host+":"+port+"...: ",err.code);
		if ( err.code == "ECONNREFUSED" && self.opts.autoReconnect ) {
			// Try connecting later
			process.nextTick(function(){
				setTimeout(function(){
					return self.connect(port,host,callback);
...
function _identifyMember(member,name,membersHeKnows) {
	...
	[name].concat(met).forEach(function(name){
		if ( self._msgQueueFor[name] ) {
			self._debug(4,"We have "+self._msgQueueFor[name].length+" messages for '"+name+"'");
			while ( self._msgQueueFor[name].length > 0 ) {
				var msg = self._msgQueueFor.shift();
