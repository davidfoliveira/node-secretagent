...
function MessageStream(format,con) {
...
//				this.msgBuf = new Buffer(this.msgBufTotal);
			...
			if ( this.msgBufMissing == 0 ) {
				this.status = "header";
				this.headerBufGot = 0;
				this.emit('message', (this.format == "binary") ? this.msgBuf : (this.format == "JSON") ? JSON.parse(this.msgBuf.toString('utf8')) : this.msgBuf.toString('utf8') );
				this.msgBuf = null;
			}
...
function sendMessage(msg) {
	...
	// Convert to JSON
	if ( typeof msg != "string" && this.format == "JSON" )
...
// End the stream
function end() {
	return (this.con != null) ? this.con.end() : null;
}
